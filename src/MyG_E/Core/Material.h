...
{
public:
	Material(Texture* diff = nullptr, Texture* spec = nullptr, float shi = 32.0f);
	Material(Material const& other);
	Material& operator=(Material const& other);
	~Material() = default;

	inline Texture const* get_diffuse() const { return m_diffuse.get(); }
	...
	inline Texture const* get_specular() const { return m_specular.get(); }
