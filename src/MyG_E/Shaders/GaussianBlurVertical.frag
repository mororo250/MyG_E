...
void main()
{
	...
	float offset = 1.0f / textureSize(u_texture, 0).y;
	float delta = textureSize(u_texture, 0).y / textureSize(u_texture, 0).x;
	vec3 g; 
	g.x = 1.0f / (sqrt(2.0f * PI) * u_sigma); 
	g.y = exp(-0.5f * delta * delta / (u_sigma * u_sigma)); 
	g.z = g.y * g.y; 
	...
	for (uint i = 0u; i < u_kernel_size / 2u; i++) 
	{
		...
		color += texture(u_texture, vec2(proj_coords.x, proj_coords.y + offset)).xyz * g.x;
		color += texture(u_texture, vec2(proj_coords.x, proj_coords.y - offset)).xyz * g.x;
		offset += offset;
	}
